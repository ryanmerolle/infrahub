...
# Both kinds are branch-agnostic, so every edge of theirs lives on the global branch.
...
WITH node, root, part_of
  ORDER BY part_of.branch_level DESC, part_of.from DESC, part_of.status ASC
...
CALL (node) {
    ...
    WITH r1, r2, account_part_of
      ORDER BY r1.branch_level DESC, r1.from DESC, r1.status ASC,
               ...
               account_part_of.branch_level DESC, account_part_of.from DESC, account_part_of.status ASC
    WITH head(collect([
