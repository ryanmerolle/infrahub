...
class NodeDeleteValidator:
    ...
    async def _analyze_delete_dependencies(self, start_nodes: Iterable[Node], at: Timestamp | str | None) -> set[str]:
        ...
        general_identifiers = [
            full_id for full_id in full_relationship_identifiers if full_id not in internal_cascade_identifiers
        ]

        peers_datas: list[RelationshipPeersData] = []
        if general_identifiers:
            peers_datas += await self._get_peers(
                full_identifiers=general_identifiers, at=at, excluded_namespaces=[INTERNAL_NAMESPACE]
            )
