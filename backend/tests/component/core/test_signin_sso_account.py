...
async def test_login_recovers_from_an_orphaned_identity(
    db: InfrahubDatabase, default_branch: Branch, register_core_models_schema: SchemaBranch
) -> None:
    """A deployment upgraded from a release that orphaned identities must still let the user in.

    Node.delete leaves the identity behind, which is the state those releases produced. The login
    has to drop that identity and start over, since the uniqueness constraint on
    (sub, provider_name, protocol) would otherwise reject the replacement.
    """
    identity = ExternalIdentity(
    ...
    orphans = await NodeManager.query(
        db=db, schema=InfrahubKind.EXTERNALIDENTITY, filters={"sub__value": "sub-orphaned-001"}
    )
    assert len(orphans) == 1
    orphan_id = orphans[0].get_id()
    assert await orphans[0].account.get_peer(db=db) is None
    ...
    identity_nodes = await NodeManager.query(
        db=db,
        schema=InfrahubKind.EXTERNALIDENTITY,
        filters={"sub__value": "sub-orphaned-001", "provider_name__value": "provider1", "protocol__value": "oidc"},
    )
    assert len(identity_nodes) == 1
    assert identity_nodes[0].get_id() != orphan_id
    linked_account = await identity_nodes[0].account.get_peer(db=db)
