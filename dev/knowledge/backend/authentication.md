...
`Migration077` deletes the identities and tokens that earlier releases orphaned. It does not make the login path above redundant: the graph-version check only logs a warning, so the server serves logins against a database whose migration has not run yet, and that is the window in which a locked-out user retries.
